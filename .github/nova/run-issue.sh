#!/usr/bin/env bash
# Nova — traitement d’une issue dans le runner.
# Claude DÉCIDE et CODE ; ce script fait les effets de bord git/GitHub de façon déterministe.
#
# Env attendu :
#   ISSUE_NUMBER            — numéro de l’issue
#   CLAUDE_CODE_OAUTH_TOKEN — auth Claude (abonnement)
#   GH_TOKEN                — token GitHub (effets gh/git, retiré de l’env de Claude)
...
block_issue() {
  local msg="$1"
  gh issue comment "$N" --body "$msg" || echo "::warning::commentaire impossible sur #$N"
  gh issue edit "$N" --add-label "nova:blocked" || echo "::warning::label impossible sur #$N"
}
...
if ! gh issue view "$N" --json number,title,body,labels,comments > "$OUT/issue.json"; then
  echo "::error::lecture de l’issue #$N impossible"; exit 1
fi
...
\`bin/rails db:prepare\` a échoué dans le runner ; je n’ai pas pu préparer la base de test, donc je n’ai rien tenté. (Problème d’infra, pas de l’issue.) Détails dans le log du run GitHub Actions."
  exit 0
fi
echo "::endgroup::"
...
{
  cat "$WS/.github/nova/triage-and-build.md"
  echo
...
# il ne peut pas pousser/ouvrir de PR ni agir sur GitHub pendant l’exécution du modèle).
echo "::group::Exécution Claude (issue #$N)"
env -u GH_TOKEN claude -p "$(cat "$OUT/prompt.md")" \
  --dangerously-skip-permissions \
  --max-turns 80 \
  > "$OUT/claude.log" 2>&1
CLAUDE_RC=$?
echo "claude exit=$CLAUDE_RC (log conservé dans le run, non publié)"
echo "::endgroup::"
...
if [ -f "$RESULT" ] && jq empty "$RESULT" 2>/dev/null; then
  ...
  echo "::warning::result.json absent ou illisible pour #$N"
fi
echo "Verdict Nova : $STATUS"
...
_Réponds dans l’issue puis retire le label \`nova:blocked\` pour que je la reprenne la nuit suivante._"
  ...
  built)
    ...
    # Closes #N uniquement dans le corps de PR (pas dans le commit).
    if ! git commit -q -m "$(printf 'Nova: issue #%s\n\n%s' "$N" "$SUMMARY")"; then
      block_issue "🌙 **Nova — commit impossible** (voir log du run). Issue laissée bloquée."
      exit 0
    fi
    if ! git push -u origin "$BRANCH" --force-with-lease; then
      block_issue "🌙 **Nova — push impossible** (voir log du run). Issue laissée bloquée."
      exit 0
    fi

    PR_BODY="$(printf 'Traitement automatique de #%s par **Nova** (agent nocturne).\n\n## Résumé\n%s\n\n## Tests\n%s\n\n## Non testé / à vérifier en review\n%s\n\nCloses #%s' "$N" "$SUMMARY" "$TESTS" "$NOTES" "$N")"

    EXISTING="$(gh pr list --head "$BRANCH" --state open --json number --jq '.[0].number' 2>/dev/null)"
    if [ -n "$EXISTING" ]; then
      gh pr edit "$EXISTING" --body "$PR_BODY" || true
      echo "PR #$EXISTING déjà ouverte sur $BRANCH — mise à jour."
    ...
    gh issue edit "$N" --add-label "nova:pr-open" || echo "::warning::label pr-open impossible sur #$N"
    ;;
...
_Le log détaillé est dans le run GitHub Actions (accès mainteneurs). Label \`nova:blocked\` posé : corrige/précise puis retire-le pour relancer._"
    ;;
esac
