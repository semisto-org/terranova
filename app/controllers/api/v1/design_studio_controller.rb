  ...
  module V1
    class DesignStudioController < BaseController
      ...
      def reporting_filters
        raw = params.permit(:period, :from, :to, :project_id, :client_id, :member_id, :group_by).to_h.symbolize_keys
        from = raw[:from].presence || reporting_period_start(raw[:period])
        {
          from: from,
          to: raw[:to].presence,
          project_id: raw[:project_id].presence,
          client_id: raw[:client_id].presence,
          member_id: raw[:member_id].presence,
          group_by: raw[:group_by].presence
        }.compact
      end

      # Traduit le sélecteur de période du dashboard (30d/90d/12m/all) en date de début.
      def reporting_period_start(period)
        case period.to_s
        when '30d' then 30.days.ago.to_date.iso8601
        when '90d' then 90.days.ago.to_date.iso8601
        when '12m' then 12.months.ago.to_date.iso8601
        end
      end
