...
const statusConfig = {
  new: { label: 'Nouvelle', color: 'text-blue-700', bgColor: 'bg-blue-100', icon: <Clock className="w-3.5 h-3.5" /> },
  processing: { label: 'En préparation', color: 'text-[#EF9B0D]', bgColor: 'bg-[#fbe6c3]', icon: <Package className="w-3.5 h-3.5" /> },
  ready: { label: 'Prête', color: 'text-green-700', bgColor: 'bg-green-100', icon: <CheckCircle2 className="w-3.5 h-3.5" /> },
  'picked-up': { label: 'Retirée', color: 'text-stone-600', bgColor: 'bg-stone-100', icon: <Truck className="w-3.5 h-3.5" /> },
  cancelled: { label: 'Annulée', color: 'text-red-700', bgColor: 'bg-red-100', icon: <XCircle className="w-3.5 h-3.5" /> },
}

const priceLevelConfig = {
  solidarity: { label: 'Solidaire', color: 'text-purple-700', bgColor: 'bg-purple-100' },
  standard: { label: 'Standard', color: 'text-stone-700', bgColor: 'bg-stone-100' },
  support: { label: 'Soutien', color: 'text-[#EF9B0D]', bgColor: 'bg-[#fbe6c3]' },
}
...
export function OrderRow({ order, onView, onProcess, onMarkReady, onMarkPickedUp, onCancel }) {
  ...
  return (
    <div className="group relative">
      ...
      {/* Mobile */}
      <div className="md:hidden p-4 border border-stone-200 rounded-lg bg-white hover:shadow-md transition-shadow mb-2">
