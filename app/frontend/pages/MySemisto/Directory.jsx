...
import { Users, Loader2, Search, MapPin, List, Map } from 'lucide-react'
...
const DirectoryMap = lazy(() => import('../../my-semisto/components/DirectoryMap'))
  ...
  return (
    <MySemistoShell activeNav={myPath('/directory')}>
      ...
      {!loading && !error && contacts.length > 0 && (
        <div className="flex items-center justify-between mb-5 my-animate-section" style={{ animationDelay: '50ms' }}>
      ...
      {!loading && !error && contacts.length > 0 && view === 'list' && (
      ...
      {/* ═══ MAP VIEW ═══ */}
      {!loading && !error && contacts.length > 0 && view === 'map' && (
        <div className="my-animate-section" style={{ animationDelay: '100ms' }}>
          <Suspense
            fallback={
              <div className="flex items-center justify-center py-16 rounded-2xl border border-stone-200 bg-stone-50">
                <Loader2 size={24} className="animate-spin" style={{ color: '#2D6A4F' }} />
              </div>
            }
          >
