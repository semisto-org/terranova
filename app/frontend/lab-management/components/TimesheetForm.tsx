...
const MODES: { value: string; label: string; color: string }[] = [
...
export function TimesheetForm({
  ...
  const [mode, setMode] = useState(timesheet?.mode ?? 'Design')
  const [phase, setPhase] = useState(timesheet?.phase ?? '')
          ...
          <div className="flex-1 overflow-y-auto min-h-0 px-6 py-5 space-y-5">
            {!isEdit && (
              <>
                ...
                {/* Mode */}
                <div>
                  <span className="block text-xs font-semibold text-stone-500 uppercase tracking-wider mb-2">Mode</span>
                  <div className="flex flex-wrap gap-2">
                    {MODES.map((m) => (
                      <button key={m.value} type="button" onClick={() => setMode(m.value)}
                        className={`px-3 py-2 rounded-xl text-sm font-medium border transition-all ${mode === m.value ? `${m.color} ring-2 ring-[#5B5781]/40 ring-offset-2` : 'bg-stone-100 text-stone-600 border-transparent hover:bg-stone-200'}`}>
                        {m.label}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <input id="timesheet-phase" type="text" value={phase}
                    onChange={(e) => setPhase(e.target.value)} className={inputBase} placeholder="Ex: Conception, Réalisation..." />
                </div>
